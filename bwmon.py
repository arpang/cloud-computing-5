...
class BandwidthMonitor(app_manager.RyuApp):
    ...
    def statsReplyHandler(self, ev):
        ...
        if name in self.topo.edgeSwitches:
            switch = self.topo.edgeSwitches[name]
            for stat in body:
                node_name = self.topo.ports[name][stat.port_no]
                if node_name in switch.neighbors:
                    self.bwstats.addHostBwStat(node_name, stat.tx_bytes, stat.rx_bytes)
